...
//下面是用户登陆时把token插入数据库的代码
    public static function saveToken($user_id, $_server_id) {
        ...
        if (!empty($_tokenData)) {
            if (empty($_tokenData['token'])) {
                $_sql = "update " . $base->table_name("user_token") . " "
                        . "set `token`='" . $token . "' , `time_out`=" . $time_out . " "
                        . "where `user_id`=" . $user_id . " and server_id=" . $_server_id . ";";
            } else {
            ...
            $_sql = "insert into " . $base->table_name("user_token") . " (`token` , `time_out`,`user_id`,`server_id`) "
                    . "values('" . $token . "'," . $time_out . "," . $user_id . "," . $_server_id . ");";
        }
        $base->query($_sql);
        return $token;
    }
