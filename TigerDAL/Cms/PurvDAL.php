...
class PurvDAL {
    ...
    public static function getAll($currentPage, $pagesize, $keywords) {
        ...
        $sql = "select * from " . $base->table_name("purv") . " where 1=1 " . $where . " order by edit_time desc limit " . $limit_start . "," . $limit_end . " ;";
    ...
    public static function getTotal($keywords) {
        ...
        $sql = "select count(1) as total from " . $base->table_name("purv") . " where 1=1 " . $where . " limit 1 ;";
    ...
    public static function getOne($id) {
        $base = new BaseDAL();
        $sql = "select * from " . $base->table_name("purv") . " where 1=1 and id=" . $id . "  limit 1 ;";
    ...
    public static function delete($id) {
        $base = new BaseDAL();
        $sql = "delete from " . $base->table_name('purv') . " where id=" . $id . " ;";
    ...
    public static function tree($cat_id = 0, $level = 0, $is_show_all = true) {
        ...
        $sql = "select c.*,c.name, COUNT(s.id) AS has_children " .
                " from " . $base->table_name('purv') . " as c " .
                "left join " . $base->table_name('purv') . " as s on s.parent_id=c.id " .
                " GROUP BY c.id " .
                " order by c.parent_id asc,c.code asc";
        $cate = $base->getFetchAll($sql);

        $options = self::purv_options($cat_id, $cate);
    ...
    /** 排序分类树 */
    static function purv_options($spec_cat_id, $arr) {
