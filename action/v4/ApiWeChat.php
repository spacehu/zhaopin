...
class ApiWeChat extends RestfulApi {
    ...
    function getWeChatInfo() {
        try {
            $this->openid=!empty($this->header['openid'])?$this->header['openid']:'';
            LogDAL::saveLog("DEBUG", "INFO", json_encode($this->get));
            if(!empty($this->get['cmod'])&&$this->get['cmod']=='sign'){
                $this->beforeSign();
            }else{
                $this->beforeWeb();
            }
    ...
    public function beforeWeb() {
        ...
        if (!empty($this->openid)) {
        ...
        }else{                             //如果$_SESSION中没有openid，说明用户刚刚登陆，就执行getCode、getOpenId、getUserInfo获取他的信息  
            //获取get的code
            $this->code = $this->getCode();
            ...
            if (!empty($userInfo) && !empty($userInfo['openid'])) {
                ...
                if (empty($result)) {
                    ...
                    //数据记录到db中 微信用户表
                    $result=$wechat->insertData($_data);
                }
        ...
        $_res = $UserInfoDAL->getUserInfo($result['user_id'],$result['nickname']);
        if(empty($result['user_id'])){
            $row['user_id']=$_res['id'];
            $wechat->updateWeChatUserInfo($result['id'],$row);
        }
        if (!empty($_res)) {
